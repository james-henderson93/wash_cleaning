...
anonymise.cleaned.data <- function(dir, name = NULL, anonymise_cols = NULL) {
  name <- ifelse(is.null(name), "data", name)
  data <- read.csv(sprintf("%s/%s_cleaned.csv", dir, name), stringsAsFactors = F, encoding = "UTF-8")
  if (is.null(anonymise_cols)) {
    anonymise_cols <- c(grep("*contact*",names(data)),
                        grep("*gpslocation*",names(data)))
  }
  data_ano <- data[,-anonymise_cols]
  write.csv(data_ano,sprintf("%s/%s_cleaned_anonymised.csv", dir, name), row.names = F, fileEncoding = "UTF-8")
} 
remove.contact.cleaned.data <- function(dir, name = NULL, anonymise_cols = NULL) {
  name <- ifelse(is.null(name), "data", name)
  data <- read.csv(sprintf("%s/%s_cleaned.csv", dir, name), stringsAsFactors = F, encoding = "UTF-8")
  if (is.null(anonymise_cols)) {
    anonymise_cols <- c(grep("*contact*",names(data)))
  ...
  data_ano <- data[,-anonymise_cols]
  write.csv(data_ano,sprintf("%s/%s_cleaned_contact_removed.csv", dir, name), row.names = F, fileEncoding = "UTF-8")
} 
...
log.cleaning.change.extended <- function(data, psu, uuid, action,  
                                         question.name=NULL, new.value=NULL, issue=NULL,
                                dir) {
  action <- ifelse(action == "c", "change", ifelse(action == "d", "deletion", ifelse(action == "f", "flag", action)))
  if (!action %in% c("change", "deletion", "flag")) {
  ...
  if (file.exists(sprintf("%s/cleaning_logbook.csv",dir))){
    ...
    log <- data.frame(uuid = character(), governorate = character(), log_date = character(), 
                      sub_district = character(), district = character(),
  ...
  for (i in 1:length(uuid)){
    ...
    log$governorate[nrow(log)] <- data$governorate_wash[row_nr]
    log$log_date[nrow(log)] <- format(Sys.Date(), "%d-%m-%Y")
    log$sub_district[nrow(log)] <- data$sub_district[row_nr]
    log$district[nrow(log)] <- NA
    log$enumerator[nrow(log)] <- data$enumerator_num[row_nr]
    log$action[nrow(log)] <- action
    log$issue[nrow(log)] <- issue
    if (action != "deletion") {
...
execute.cleaning.changes <- function(dir, filenameparent = "data", filenamechild = "child", uuid_column=NULL) {
  if (!file.exists(sprintf("%s/cleaning_logbook.csv",dir))){
...
set.up.cleaning.dir <- function(dir, name = NULL) {
  ...
  file.copy(sprintf("%s/data_cleaned.csv", old_dirs[length(old_dirs)]), 
            sprintf("%s/%s/data_cleaned_old.csv", main_dir, date))
